...
while getopts "hzrRmk" arg; do
    ...
    r)
        ...
        do
            ...
            ssh -f $ROS2_PONG "
                cd $WORKING_DIR/$PERF_DIR
                CYCLONEDDS_URI='$ROS2_PONG_CYCLONEDDS_URI' NICE=$ROS2_PONG_NICE CPUS=$ROS2_PONG_CPUS MSGS=$s ./run-single-process.sh -or
            "
            PONG_PID=$!
            sleep 1
            ...
            ssh $ROS2_PING "
                cd $WORKING_DIR/$PERF_DIR
                CYCLONEDDS_URI='$ROS2_PING_CYCLONEDDS_URI' DURATION=$TEST_TIME NICE=$ROS2_PING_NICE CPUS=$ROS2_PING_CPUS MSGS=$s ./run-single-process.sh -ir
            "
            PING_PID=$!
            sleep 1
            wait $PING_PID
    ...
    R)
        ...
        do
            ...
            ssh -f $ROS_MASTER "
                cd $WORKING_DIR/$PERF_DIR
                ROS_IP=$ROS_IP_MASTER ROS_MASTER_URI=$ROS_MASTER_URI NICE=$ROS_MASTER_NICE CPUS=$ROS_MASTER_CPUS ./run-single-process.sh -bR
            "
            MASTER_PID=$!
            sleep 3
            ...
            ssh -f $ROS_PONG "
                cd $WORKING_DIR/$PERF_DIR
                ROS_IP=$ROS_IP_PONG ROS_MASTER_URI=$ROS_MASTER_URI NICE=$ROS_PONG_NICE CPUS=$ROS_PONG_CPUS ./run-single-process.sh -oR
            "
            PONG_PID=$!
            sleep 1
            ...
            ssh $ROS_PING "
                cd $WORKING_DIR/$PERF_DIR
                ROS_IP=$ROS_IP_PING ROS_MASTER_URI=$ROS_MASTER_URI DURATION=$TEST_TIME NICE=$ROS_PING_NICE CPUS=$ROS_PING_CPUS MSGS=$s ./run-single-process.sh -iR
            "
            PING_PID=$!
            sleep 1
            wait $PING_PID
            ...
            rsync -azv "$ROS_PINGN:~/$WORKING_DIR/$PERF_DIR/logs/*.csv" $TEST_LOGS/
